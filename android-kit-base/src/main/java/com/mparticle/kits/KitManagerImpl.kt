...
open class KitManagerImpl(//================================================================================
    ...
    fun getIntegrationAttributes(kitIntegration: KitIntegration?): Map<String, String>? {
        return mCoreCallbacks.getIntegrationAttributes(kitIntegration?.configuration?.kitId ?: 0)
    }
    ...
    override fun updateKits(jsonArray: JSONArray?): KitsLoadedCallback? {
        val callback = KitsLoadedCallback()
        runOnKitThread {
            kitConfigurations = parseKitConfigurations(jsonArray)
            runOnMainThread {
    ...
    private fun initializeKit(activeKit: KitIntegration?) {
        ...
        if (activeKit is ActivityListener) {
            val activityWeakReference = getCurrentActivity()
    ...
    override fun getKitStatus(): Map<Int?, KitStatus?> {
    ...
    override fun getSurveyUrl(
        ...
        var userAttributeLists: Map<String, List<String>>? = userAttributeLists
    ...
    override fun getSupportedKits(): Set<Int>? {
                ...
                try {
                    if (!provider.isDisabled) {
                        val onPushRegistration =
                            (provider as PushListener).onPushRegistration(instanceId, senderId)
                        mCoreCallbacks.kitListener.onKitApiCalled(
                            provider.configuration.kitId,
                            onPushRegistration,
                            instanceId,
    ...
    override fun setUserAttribute(key: String, value: String, mpid: Long) {
        ...
        for (provider in providers.values) {
            try {
                setUserAttribute(provider, key, value, mpid)
            } catch (e: Exception) {
    ...
    override fun removeUserIdentity(id: IdentityType?) {
        if (mDataplanFilter.isUserIdentityBlocked(id)) {
            return
        }
        for (provider in providers.values) {
            try {
                if (provider is AttributeListener && !provider.isDisabled) {
                    (provider as AttributeListener).removeUserIdentity(id)
                }
            } catch (e: Exception) {
    ...
    override fun getCurrentActivity(): WeakReference<Activity>? {
        return mCoreCallbacks.currentActivity
    }
    ...
    override fun getAttributionResults(): MutableMap<Int, AttributionResult> {
                ...
                if (provider is IdentityListener && !provider.isDisabled) {
                    (provider as IdentityListener).onIdentifyCompleted(
                        FilteredMParticleUser.getInstance(
                            user,
                            provider
                        ), FilteredIdentityApiRequest(request, provider)
                    )
                }
            } catch (e: Exception) {
    ...
    override fun onConsentStateUpdated(oldState: ConsentState?, newState: ConsentState?, mpid: Long) {
    ...
    private fun parseKitConfigurations(kitConfigs: JSONArray?): List<KitConfiguration> {
        var kitConfigs: JSONArray? = kitConfigs
